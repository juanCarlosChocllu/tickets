...
export class AreasService {
  constructor(
    @InjectModel(Area.name) private readonly AreaSchema: Model<Area>,
  ) {}
  async create(createAreaDto: CreateAreaDto) {
    const areaExiste = await this.AreaSchema.findOne({
  ...
  async findOne(id: string) {
    const area = await this.validarArea(id);
    return area;
  }

  async update(id: string, updateAreaDto: UpdateAreaDto) {
    await this.validarArea(id);
  ...
  public async validarArea(id: string) {
    const area = await this.AreaSchema.findOne({ _id: id, flag: Flag.nuevo });
    if (!area) {
